...
#define SENSOR_BAUDRATE     230400     /**< Configured for 0.00% clock error with 14.7456 MHz crystal */
...
static void init_board_hardware(void) {
    ...
    for (int i = 0; i < 12; i++) {
        ...
        esp_err_t ret = sc16is752_configure_uart(sensor->chip_idx, sensor->channel, SENSOR_BAUDRATE);
        if (ret == ESP_OK) {
...
static void radar_aggregator_task(void *pvParameters) {
    ...
    while (1) {
        ...
        for (int i = 0; i < 12; i++) {
            if (!s_sensor_telemetry[i].is_online) continue;

            const sensor_map_t *sensor = &SENSOR_MAP[i];
            
            // Read pending serial bytes from the UART bridge
            int rx_len = sc16is752_read_bytes(sensor->chip_idx, sensor->channel, rx_buffer, sizeof(rx_buffer));
            ...
            if (rx_len > 0) {
                ...
                if (ld2410c_parse_target_data(rx_buffer, rx_len, &target) == ESP_OK) {
                    ...
                    ESP_LOGD(TAG, "[%s] State: %d, MovDist: %d cm, StatDist: %d cm", 
                             sensor->name, target.state, target.moving_distance_cm, target.stationary_distance_cm);
                }
            }
        }
...
static void telemetry_publish_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (!mqtt_ready && ethernet_is_connected()) {
            ESP_LOGI(TAG, "Ethernet connection verified. Initializing MQTT client...");
            if (mqtt_init() == ESP_OK) {
                mqtt_ready = true;
            }
        }
        ...
        // Delay for 100ms (10Hz reporting speed for high-performance escape room triggers)
...
void app_main(void) {
    ...
    xTaskCreatePinnedToCore(radar_aggregator_task, "radar_aggregator", 4096, NULL, 5, NULL, 0);

    // Spawn periodic network publisher thread
    xTaskCreatePinnedToCore(telemetry_publish_task, "telemetry_publish", 4096, NULL, 4, NULL, 1);
}
