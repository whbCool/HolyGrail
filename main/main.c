...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(uart_set_pin(LD2410C_UART_PORT, 
                                 LD2410C_TX_PIN, 
                                 LD2410C_RX_PIN, 
                                 UART_PIN_NO_CHANGE, 
                                 UART_PIN_NO_CHANGE));
    ...
    while (1) {
        if (ld2410c_read_data_frame(ld, buffer, sizeof(buffer), &length) == ESP_OK) {
            ...
            if (ld2410c_parse_target_data(buffer, length, &target) == ESP_OK) {
                ...
                if (target.state == LD2410C_TARGET_MOVING || target.state == LD2410C_TARGET_BOTH) {
                    ESP_LOGI(TAG, "  -> Moving Target Distance: %d cm (Energy: %d)", 
                             target.moving_distance_cm, target.moving_energy);
                }
                if (target.state == LD2410C_TARGET_STATIONARY || target.state == LD2410C_TARGET_BOTH) {
