/**
 * @file sc16is752.c
 * @brief SPI driver implementation for the SC16IS752 Dual UART-to-SPI bridge controller.
 */
...
esp_err_t sc16is752_init(const sc16is752_hw_config_t *config) {
    ...
    gpio_config_t io_conf = {
        ...
        .pin_bit_mask = ((1ULL << s_hw_config.pin_a0) |
                         (1ULL << s_hw_config.pin_a1) |
                         (1ULL << s_hw_config.pin_a2) |
                         (1ULL << s_hw_config.pin_e3) |
    ...
    // Keep CD74HC138 disabled (E3 LOW) and MISO buffer disabled (OE HIGH) by default
    ...
    spi_device_interface_config_t devcfg = {
        .clock_speed_hz = 10 * 1000 * 1000,      // 10 MHz SPI clock (safe & compliant with both chips)
        .mode = 0,                               // SPI Mode 0
    ...
    esp_err_t ret = spi_bus_add_device(s_hw_config.spi_host, &devcfg, &s_spi_handle);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to add SC16IS752 to SPI bus: %s", esp_err_to_name(ret));
...
esp_err_t sc16is752_read_reg(uint8_t chip_idx, sc16is752_channel_t channel, uint8_t reg_offset, uint8_t *out_val) {
    ...
    // Read flag (bit 7) = 1, Register offset (bits 6..3), Channel offset (bits 2..1)
    uint8_t cmd_byte = 0x80 | ((reg_offset & 0x0F) << 3) | ((channel & 0x03) << 1);
    ...
    if (ret == ESP_OK) {
        *out_val = rx_data[1]; // The second byte shifted out contains the register data
    }
    return ret;
}
...
esp_err_t sc16is752_write_reg(uint8_t chip_idx, sc16is752_channel_t channel, uint8_t reg_offset, uint8_t val) {
    ...
    // Write flag (bit 7) = 0, Register offset (bits 6..3), Channel offset (bits 2..1)
...
esp_err_t sc16is752_configure_uart(uint8_t chip_idx, sc16is752_channel_t channel, uint32_t baudrate) {
    ...
    uint8_t lcr_val = 0;
    ESP_ERROR_CHECK(sc16is752_read_reg(chip_idx, channel, SC16IS752_REG_LCR, &lcr_val));
    lcr_val |= SC16IS752_LCR_DLAB;
    ESP_ERROR_CHECK(sc16is752_write_reg(chip_idx, channel, SC16IS752_REG_LCR, lcr_val));

    // 2. Calculate integer divisor
    // Baud Rate = XTAL_FREQ / (16 * Divisor) -> Divisor = XTAL_FREQ / (16 * Baud Rate)
    // Prescaler defaults to 1 (divisor divided by 1)
    uint32_t divisor = SC16IS752_XTAL_FREQ / (16 * baudrate);
    uint8_t dll_val = divisor & 0xFF;
    uint8_t dlh_val = (divisor >> 8) & 0xFF;

    ESP_ERROR_CHECK(sc16is752_write_reg(chip_idx, channel, SC16IS752_REG_DLL, dll_val));
    ESP_ERROR_CHECK(sc16is752_write_reg(chip_idx, channel, SC16IS752_REG_DLH, dlh_val));

    // 3. Disable DLAB and set UART framing (8N1: 8 data bits, no parity, 1 stop bit)
    // 8 data bits: LCR[1..0] = 11 (0x03)
    // 1 stop bit: LCR[2] = 0
    // No parity: LCR[5..3] = 000
    lcr_val = 0x03; // DLAB = 0, 8N1
    ESP_ERROR_CHECK(sc16is752_write_reg(chip_idx, channel, SC16IS752_REG_LCR, lcr_val));

    // 4. Enable and reset FIFOs, set trigger level to 8 bytes (so we get fewer interrupts)
    // FCR[0]: Enable FIFO = 1
    // FCR[1]: Reset RX FIFO = 1
    // FCR[2]: Reset TX FIFO = 1
    // FCR[7..6]: RX trigger level = 01 (8 bytes)
    uint8_t fcr_val = 0x01 | 0x02 | 0x04 | (0x01 << 6);
    ESP_ERROR_CHECK(sc16is752_write_reg(chip_idx, channel, SC16IS752_REG_FCR, fcr_val));
    ...
    ESP_ERROR_CHECK(sc16is752_write_reg(chip_idx, channel, SC16IS752_REG_IER, ier_val));

    ESP_LOGD(TAG, "UART successfully configured on Chip %d Channel %d (Divisor: %ld)", 
...
int sc16is752_write_bytes(uint8_t chip_idx, sc16is752_channel_t channel, const uint8_t *data, size_t len) {
    ...
    while (bytes_written < len) {
        ...
        if (sc16is752_read_reg(chip_idx, channel, SC16IS752_REG_TXLVL, &tx_level) != ESP_OK) {
            break;
        }
        ...
        for (size_t i = 0; i < write_chunk; i++) {
            if (sc16is752_write_reg(chip_idx, channel, SC16IS752_REG_THR, data[bytes_written + i]) != ESP_OK) {
                break;
            }
        }
...
uint8_t sc16is752_get_rx_level(uint8_t chip_idx, sc16is752_channel_t channel) {
    uint8_t rx_level = 0;
    if (sc16is752_read_reg(chip_idx, channel, SC16IS752_REG_RXLVL, &rx_level) != ESP_OK) {
        return 0;
    }
    return rx_level;
}
...
int sc16is752_read_bytes(uint8_t chip_idx, sc16is752_channel_t channel, uint8_t *buffer, size_t max_len) {
    ...
    for (size_t i = 0; i < read_chunk; i++) {
        uint8_t byte = 0;
        if (sc16is752_read_reg(chip_idx, channel, SC16IS752_REG_RHR, &byte) == ESP_OK) {
            buffer[i] = byte;
            bytes_read++;
        } else {
            break;
